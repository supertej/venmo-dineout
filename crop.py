...
def find_components(edges, max_components=10):
    ...
    while count > max_components:
        ...
        components = cv2.connectedComponentsWithStats(np.uint8(dilated_image))
        count = components[0]
    if imshow:
        cv2.imshow("Edged", resize(edges, height=650))
        cv2.imshow("Edged dilated", resize(255 * dilated_image, height=650))
        cv2.waitKey(0)
